...
from PyQt5.QtWidgets import (    
    QDialog, # это базовый класс диалогового окна
    QTableWidget, QLineEdit, QComboBox, QCalendarWidget
)
...
class AddRequest(QMainWindow):
    def __init__(self, BButton, Sbutton, lineIDrequest,
        ...
        conn1 = sqlite3.connect("newbd")
        cur1 = conn1.cursor()
        cur1.execute(f'select IDorgTechType from orgTechTypes;')
        IDorgTechType1 = cur1.fetchall()
        for el in IDorgTechType1:
            print(el)
            lineorgTechTypeID.addItem(str(el[0]))

        conn1.commit()
        conn1.close()
        print(self.orgTechTypeIDLine.currentText())
    ...
    def SaveFunction(self):        
        ...
        orgTechTypeID = self.orgTechTypeIDLine.currentText()
        print(orgTechTypeID)
...
class Authorization(QMainWindow):
    ...
    def AddRequestButfunction(self):
        ...
        self.orgTechTypeIDLine = self.findChild(QComboBox,'orgTechTypeIDLine')
